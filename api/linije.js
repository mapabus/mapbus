...
try {
const gradskeResponse = await fetch('/api/trips_gradske.txt');
const gradskeText = await gradskeResponse.text();
tripsGradske = parseGTFS(gradskeText);
const response = await fetch('/api/trips.txt');
const text = await response.text();
trips = parseGTFS(text);
} catch (error) {
console.error('❌ Greška pri učitavanju trips:', error);
}
}
async function loadStopTimes() {
try {
const gradskeResponse = await fetch('/api/stop_times_gradske.js');
const gradskeData = await gradskeResponse.json();
stopTimesGradske = gradskeData;
const response = await fetch('/api/stop_times.txt');
const text = await response.text();
stopTimes = parseGTFS(text);
} catch (error) {
console.error('❌ Greška pri učitavanju stop_times:', error);
}
}
async function loadStops() {
try {
stationsMap = {};
const gradskeResponse = await fetch('/api/stops_gradske.txt');
const gradskeText = await gradskeResponse.text();
const gradskeStops = parseGTFS(gradskeText);
gradskeStops.forEach(stop => {
const normId = normalizeStopId(stop.stop_id);
stationsMap[normId] = {
name: stop.stop_name,
coords: [parseFloat(stop.stop_lat), parseFloat(stop.stop_lon)]
};
});
const response = await fetch('/api/stops.txt');
const text = await response.text();
const stops = parseGTFS(text);
stops.forEach(stop => {
const normId = normalizeStopId(stop.stop_id);
if (!stationsMap[normId]) {
stationsMap[normId] = {
name: stop.stop_name,
coords: [parseFloat(stop.stop_lat), parseFloat(stop.stop_lon)]
};
...
console.error('❌ Greška pri učitavanju stops:', error);
}
}
...
let selectedTrips = tripsGradske.filter(t => t.route_id === normalizeRouteId(routeId));
if (selectedTrips.length === 0) {
selectedTrips = trips.filter(t => t.route_id === normalizeRouteId(routeId));
}
return selectedTrips;
}
function getStopTimesForTrip(tripId) {
let selected = stopTimesGradske.filter(st => st.trip_id === tripId);
if (selected.length === 0) {
selected = stopTimes.filter(st => st.trip_id === tripId);
}
return selected;
}
function getLastStopForTrip(tripId) {
let selectedStopTimes = getStopTimesForTrip(tripId);
if (selectedStopTimes.length === 0) return null;
selectedStopTimes.sort((a, b) => parseInt(b.stop_sequence) - parseInt(a.stop_sequence));
return normalizeStopId(selectedStopTimes[0].stop_id);
}
...
routeNamesMap = {};
Object.keys(routeMappingData).forEach(key => {
const left = key.split(':')[0].trim();
routeNamesMap[left] = key;
});
} catch (error) {
...
function drawRoutesAndStations() {
routeLayer.clearLayers();
stationLayer.clearLayers();
izabraneLinije.forEach(routeId => {
const mappingKeys = Object.keys(routeMappingData).filter(key => key.split(':')[0].trim() === routeId);
mappingKeys.forEach(mappingKey => {
const shapeId = routeMappingData[mappingKey];
const shape = shapesGradskeData[shapeId] || shapesData[shapeId];
if (!shape) return;
const points = shape.map(p => [p.lat, p.lon]);
const tripsForRoute = getTripsForRoute(routeId);
const trip = tripsForRoute.find(t => t.shape_id === shapeId);
if (!trip) return;
const tripId = trip.trip_id;
const destId = getLastStopForTrip(tripId);
if (!destId) return;
const uniqueDirKey = `${routeId}_${destId}`;
...
L.polyline(points, {color: color, weight: 3, opacity: 0.8}).addTo(routeLayer);
const stopTimes = getStopTimesForTrip(tripId);
stopTimes.sort((a, b) => parseInt(a.stop_sequence) - parseInt(b.stop_sequence));
stopTimes.forEach(st => {
const stopId = normalizeStopId(st.stop_id);
const station = stationsMap[stopId];
if (!station) return;
const markerHtml = `<div style="background: ${color}; width: 8px; height: 8px; border-radius: 50%; border: 1px solid white;"></div>`;
...
const destName = stationsMap[destId] ? stationsMap[destId].name : destId;
const popup = `
<div class="popup-content">
<div class="popup-row"><span class="popup-label">Stanica:</span> <b>${station.name}</b></div>
<div class="popup-row"><span class="popup-label">Smer:</span> <span style="color:$$ {color}; font-weight:bold;"> $${trip.trip_headsign || mappingKey.split(':')[1].trim()} (ka ${destName})</span></div>
</div>
`;
L.marker(station.coords, {icon: icon}).bindPopup(popup).addTo(stationLayer);
});
});
});
}
